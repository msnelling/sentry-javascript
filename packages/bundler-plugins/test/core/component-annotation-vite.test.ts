...
async function annotateWithVite(
  ...
  const hooks = createViteComponentNameAnnotateHooks(ignoredComponents, getParseAstAsync, injectIntoHtml);
...
export const List = <T,>(props: Props<T>) => {
  ...
  return (
    ...
    const viteResult = await annotateWithVite(code, id, ignoredComponents, getParseAstAsync, true);
